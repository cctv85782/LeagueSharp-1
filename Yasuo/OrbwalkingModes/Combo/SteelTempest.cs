namespace Yasuo.OrbwalkingModes.Combo
{
    ...
    internal class SteelTempest : Child<Combo>
    {
        ...
        public void OnUpdate(EventArgs args)
        {
            ...
            // Stacking
            if (this.Menu.SubMenu(this.Name + "stacksettings").Item("Stacking").GetValue<bool>() && !this.ProviderQ.HasQ3())
            {
                var units = this.ProviderE.GetUnits(Variables.Player.ServerPosition);

                var stacksettings = this.Menu.SubMenu(this.Name + "stacksettings");

                switch (stacksettings.Item("Mode").GetValue<StringList>().SelectedIndex)
                {
                    case 0:
                        ...
                        if (Variables.Player.ServerPosition.Distance(HeroManager.Enemies.Where(x => !x.IsDead || !x.IsZombie).MinOrDefault(x => x.Distance(Variables.Player)).ServerPosition)
                            <= stacksettings.Item("MinDistance").GetValue<Slider>().Value)
                        {
                            if (Variables.Spells[SpellSlot.Q].Cooldown
                                >= stacksettings.Item("MinCooldownQ").GetValue<Slider>().Value)
                            {
                                if (units.Count > 0)
                                {
        ...
        {
            var pred = ProviderQ.GetPrediction(target, aoe);
