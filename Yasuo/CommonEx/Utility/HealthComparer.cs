namespace Yasuo.CommonEx.Utility
{
    ...
    internal class HealthComparer
    {
        ...
        {
            return this.Evaluate(unit.HealthPercent);
        }
        ...
        {
            return this.Evaluate(units.Sum(x => x.HealthPercent) / units.Count);
        }
        ...
        {
            var healthcompareSettings = this.menu.SubMenu(this.menu.Name + "Health-Comparer");

            switch (healthcompareSettings.Item(healthcompareSettings.Name + "Mode").GetValue<StringList>().SelectedIndex + 1)
            {
                case 1:
        ...
        private void SetupMenu()
        {
            ...
            var dynamicMenu = new DynamicMenu(this.menu, "Health-Comparer", selecter, menuArray);
