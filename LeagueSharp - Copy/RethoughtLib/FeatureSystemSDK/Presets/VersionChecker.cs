namespace RethoughtLib.FeatureSystemSDK.Presets
{
    ...
    internal class VersionChecker : ChildBase
    {
            ...
            if (this.ImplementationVersionChecker.UpdateAvailable)
            {
                ...
                else
                {
                    if (this.Menu[this.Name + "NotifyNewVersion"].GetValue<MenuBool>().Value)
                    {
                        if (!this.notified)
                        {
                            Notifications.Add(new Notification(this.AssemblyName + "- Update available!", "Please Update."));
        ...
        {
            global::RethoughtLib.Events.Events.OnUpdate -= this.OnUpdate;
        }
        ...
        protected sealed override void OnLoad(object sender, FeatureBaseEventArgs featureBaseEventArgs)
        {
            ...
            this.Menu.Add(
                new MenuBool(this.Name + "LiveCheck", "Check For new Version", true));

            this.Menu.Add(
                new MenuBool(this.Name + "NotifyNewVersion", "Notify if new Version available", true));
