namespace RethoughtLib.Bootstraps.Abstract_Classes
{
    ...
    public abstract class PlaySharpBootstrapBase : IBootstrap
    {
        ...
        public virtual void Run()
        {
            ...
            Console.WriteLine(
                $"[{GlobalVariables.DisplayName}] PlaySharpBootstrapBase: {unknownModulesCount} unknown Modules, {loadedModulesCount} loaded Modules");

            if (unknownModulesCount > 0)
            {
                Console.WriteLine(
                    $"[{GlobalVariables.DisplayName}] PlaySharpBootstrapBase: Please consider tagging and naming your unknown modules. The name must not be null or whitespace.");
            }
        }
