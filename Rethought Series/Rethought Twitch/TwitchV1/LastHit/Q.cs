        ...
        private void LogicJungleLastHit()
        {
            var units =
                MinionManager.GetMinions(
                    this.twitchQ.Spell.Range,
                    MinionTypes.All,
                    MinionTeam.Neutral,
                    MinionOrderTypes.Health).Where(x => this.twitchQ.WillReset(x)).ToList();
            var unit = units.FirstOrDefault();

            if (unit == null) return;

            this.twitchQ.Spell.Cast(unit);
        }

        private void LogicLaneLastHit()
        {
            var units =
                MinionManager.GetMinions(this.twitchQ.Spell.Range, MinionTypes.All, MinionTeam.NotAlly)
                    .Where(x => this.twitchQ.WillReset(x))
                    .ToList();
            ...
            foreach (var enemy in
                HeroManager.Enemies.Where(
                    x => !x.IsDead && (x.Distance(ObjectManager.Player) <= 1000 + this.twitchQ.Spell.Range)))
            ...
            if (unit != null) this.twitchQ.Spell.Cast(unit);
        }
