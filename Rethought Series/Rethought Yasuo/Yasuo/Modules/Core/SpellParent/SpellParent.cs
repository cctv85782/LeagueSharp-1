namespace Rethought_Yasuo.Yasuo.Modules.Core.SpellParent
{
    ...
    internal sealed class SpellParent : ParentBase, ISpellIndex
    {
        ...
        ///     The spells
        /// </summary>
        private readonly Dictionary<SpellSlot, SpellChild> spells;
        ...
        /// <param name="sender">The sender.</param>
        /// <param name="parentBaseEventArgs">The <see cref="ParentBase.ParentBaseEventArgs" /> instance containing the event data.</param>
        protected override void OnChildRemove(object sender, ParentBaseEventArgs parentBaseEventArgs)
        {
            base.OnChildRemove(sender, parentBaseEventArgs);

            var spellChild = parentBaseEventArgs.Child as SpellChild;

            if (spellChild?.Spell == null) return;

            this.spells.Remove(spellChild.Spell.Slot);
        }
