namespace Rethought_Twitch.TwitchV1.Combo
{
    ...
    internal class Q : OrbwalkingChild
    {
        ...
        /// </summary>
        /// <param name="twitchQ">The Q logic</param>
        /// <param name="damageCalculator">The damage calculator</param>
        public Q(TwitchQ twitchQ, IDamageCalculator damageCalculator)
        {
        ...
        protected override void OnLoad(object sender, FeatureBaseEventArgs featureBaseEventArgs)
        {
            ...
            this.Menu.AddItem(
                new MenuItem(this.Path + "." + "minrangetogapclose", "Don't gapclose if closer than X units").SetValue(
                    new Slider((int)ObjectManager.Player.AttackRange, 0, (int)this.twitchQ.Spell.Range)));
        }
        ...
        private Vector3 GetMovementPrediction()
        {
            ...
            var gapclosePath = this.twitchQ.GetPath(ObjectManager.Player.ServerPosition, this.target.ServerPosition);
            ...
            for (var i = 0; i < gapclosePath.Count - 1; i++)
            {
                ...
                expectedTime += gapclosePath[i].Distance(gapclosePath[i + 1]) / this.twitchQ.Spell.Speed;
            }
        ...
        private void LogicBaitEnemy()
        {
            if (!this.Menu.Item(this.Path + "." + "baitenemy").GetValue<bool>() || this.target == null
                ...
                <= this.twitchQ.Spell.Range - Baitrange) return;

            var possibleUnits =
                ObjectManager.Get<Obj_AI_Base>()
                    .Where(
                        x =>
                        this.twitchQ.WillReset(x)
                        && x.Distance(this.target) <= ObjectManager.Player.Distance(this.target));

            this.twitchQ.Spell.Cast(possibleUnits.MinOrDefault(x => x.Distance(this.target)));
        }
        ...
        {
            if (this.twitchQ.WillReset(this.target))
            {
                this.twitchQ.Spell.Cast(this.target);
            }
        }
        ...
        private void LogicPathfinding()
        {
            ...
            var path = this.twitchQ.GetPath(ObjectManager.Player.ServerPosition, end);

            if (path == null || !path.Any()) return;

            this.twitchQ.Spell.Cast(path.FirstOrDefault());
        }
