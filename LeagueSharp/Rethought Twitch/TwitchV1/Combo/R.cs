namespace Rethought_Twitch.TwitchV1.Combo
{
    ...
    internal class R : OrbwalkingChild
    {
        ...
        /// </summary>
        /// <param name="twitchR">The irelia r.</param>
        /// <param name="twitchQ">The irelia q</param>
        /// <param name="damageCalculator">The damage calculator</param>
        public R(TwitchR twitchR, TwitchQ twitchQ, IDamageCalculator damageCalculator)
        {
        ...
        {
            var prediction = this.twitchR.Spell.GetPrediction(
                target,
                true,
            ...
            {
                this.twitchR.Spell.Cast(prediction.CastPosition);
            }
        }
        ...
        private void LogicAutoCombo(Obj_AI_Base target)
        {
            if (this.DamageCalculator.GetDamage(target) < target.Health
                || (this.twitchQ.GetPath(ObjectManager.Player.ServerPosition, target.ServerPosition) == null
                    && ObjectManager.Player.Distance(target) > this.twitchQ.Spell.Range
                    && this.twitchR.GetDamage(target) * this.twitchR.EstimatedAmountInOneCombo < target.Health)) return;
