namespace Rethought_Twitch.TwitchV1.GraphGenerator
{
    ...
    internal class GraphGeneratorModule : ChildBase, IGraphGenerator<AStarNode, AStarEdge<AStarNode>>
    {
        ...
        public GraphGeneratorModule(TwitchQ twitchQ)
        {
        ...
        ///     The spell
        /// </summary>
        public TwitchQ TwitchQ { get; set; }
        ...
        public Graph<AStarNode, AStarEdge<AStarNode>> Generate(AStarNode start, AStarNode end)
        {
            ...
            foreach (var centerNode in nodes)
            {
                foreach (var neighbor in
                    nodes.Where(
                        x => centerNode != x && x.Position.Distance(centerNode.Position) <= this.TwitchQ.Spell.Range))
                {
