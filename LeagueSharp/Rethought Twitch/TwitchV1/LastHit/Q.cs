        ...
        private void LogicLaneLastHit()
        {
            var units =
                MinionManager.GetMinions(
                    this.twitchQ.Spell.Range,
                    MinionTypes.All,
                    MinionTeam.NotAlly).Where(x => this.twitchQ.WillReset(x)).ToList();
            ...
            foreach (var enemy in
                HeroManager.Enemies.Where(x => !x.IsDead && x.Distance(ObjectManager.Player) <= 1000 + this.twitchQ.Spell.Range))
            {
